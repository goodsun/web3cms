...
// Simple NFT ABI with just tokenURI method
const NFT_ABI = [
...
export const handler = async (
  ...
  try {
    ...
    const CACHE_TTL = 30 * 24 * 60 * 60; // 30 days for tokenURI
    ...
    if (!force) {
      ...
      // Check if tokenURI exists in cache and is not expired
      if (cacheResult.Item?.tokenURI && 
    ...
    // Extract metadata using common utility
    const metadata = await extractMetadataFromTokenURI(tokenURI);
    const { name = null, description = null, imageUrl = null } = metadata;
    ...
    if (error instanceof Error && error.message.includes('call revert exception')) {
      return createResponse(HTTP_STATUS.NOT_FOUND, {
        message: 'Token does not exist or contract does not support tokenURI'
      });
    }
    
    return createResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, {
      message: 'Failed to fetch tokenURI',
