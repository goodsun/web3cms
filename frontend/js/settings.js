...
document.addEventListener("DOMContentLoaded", () => {
  // Load and display current API endpoint
  displayApiEndpoint();
  ...
  // API endpoint save handler
  document
    .getElementById("saveApiEndpoint")
    .addEventListener("click", saveApiEndpoint);
});
...
function checkApiEndpoint() {
  if (!CONFIG.API_ENDPOINT) {
    showError(
      "API endpoint not configured. Please configure it on the main page first."
    );
...
async function loadSettings() {
  ...
  try {
    const response = await fetch(
      `${CONFIG.API_ENDPOINT}/settings/${SETTINGS_KEY}`
    );
...
function showForm(data) {
  ...
  document.getElementById("enableWalletConnect").checked =
    web3.enableWalletConnect || false;
  ...
  document.getElementById("requireAuthentication").checked =
    features.requireAuthentication || false;
...
function showSuccess(message) {
  ...
  if (!success) {
    ...
    success.className = "message message--success";
    document
      .querySelector("main")
      .insertBefore(success, document.getElementById("settingsForm"));
  }
...
function displayApiEndpoint() {
  ...
  // Show auto-configured endpoint if available
  if (CONFIG.IS_API_MANAGED && CONFIG.API_ENDPOINT) {
    apiEndpointInput.value = CONFIG.API_ENDPOINT;
    apiEndpointInput.setAttribute("readonly", "true");
    document.getElementById("saveApiEndpoint").style.display = "none";

    // Add note about auto-configuration
    const small = apiEndpointInput.nextElementSibling;
    small.innerHTML = "Auto-configured by deployment (read-only)";
  } else {
    // Show saved endpoint from localStorage
