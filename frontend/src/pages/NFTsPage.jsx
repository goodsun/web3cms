      ...
      try {
        ...
        promises.push(
          nftContract
            .totalSupply()
            .then((supply) => {
              fields.totalSupply = supply.toString();
            })
            .catch(() => {
              fields.totalSupply = "N/A";
            })
        );
  ...
  return (
    <div className="nfts-page">
      <div className="page-header">
        <h1>{nftInfo && nftInfo.name ? nftInfo.name : "NFTs"}</h1>
      </div>
              ...
              <div className="nft-contract-info">
                <small>
                  {loading ? (
                    ...
                    <>
                      Name: <strong>{nftInfo.name}</strong> | Symbol:{" "}
                      <strong>{nftInfo.symbol}</strong>
      ...
      <section className="config-section">
        ...
        <div className="config-grid">
          <div className="config-item">
            ...
            <div className="config-value">{isConnected ? "Yes" : "No"}</div>
          </div>
          {isConnected && (
            <>
              <div className="config-item">
                <label>Account</label>
                <div className="config-value monospace">{account}</div>
              </div>
              <div className="config-item">
                <label>Current Chain ID</label>
                <div className="config-value monospace">{chainId}</div>
              </div>
            </>
          )}
        </div>
      ...
      <section className="config-section">
        ...
        <div className="status-grid">
          <div className="status-item">
            <span
              className={`status-indicator ${
                isConnected ? "status-ok" : "status-warning"
              }`}
            >
              ●
            </span>
          ...
          <div className="status-item">
            <span
              className={`status-indicator ${
                web3Config.rpcUrls ? "status-ok" : "status-error"
              }`}
            >
              ●
            </span>
          ...
          <div className="status-item">
            <span
              className={`status-indicator ${
                web3Config.nftContract ? "status-ok" : "status-warning"
              }`}
            >
              ●
            </span>
          ...
          <div className="status-item">
            <span
              className={`status-indicator ${
                web3Config.tbaRegistry && web3Config.tbaImplementation
                  ? "status-ok"
                  : "status-warning"
              }`}
            >
